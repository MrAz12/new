...
shp = '妥︎⃟🐲'
shpe = '*妥︎⃟🕊 •*'
footerr = '_Made With FixMood_'
multi = true // Multi Prefix

nopref = false // Noprefix
fake = '𝐈𝐭𝐬𝐌𝐞 𝐅𝐢𝐱𝐌𝐨𝐨𝐝 🍂' // Fake
...
var totalChat = await Fix.chats.all()
 menu =` ${ucapanWaktu} - @${sender.split("@")[0]}👋
   ...
   *INFO OWNER*
*${shp} Owner :* @${owner}
*${shp} Instagram :* imfikkur___
*${shp} Youtube :* FixMood BOT
...
case 'storyanime':
     ...
     case 'pornhub':
                ...
                case 'steel3d':
                case 'wallgravity':
                    if (args.length == 0) return fakestatus(`Contoh: ${prefix + command} FixMood BOT`)
                    teks1 = args[0]
                    teks2 = args[1]
...
//------------------< Lainnya >-------------------
         ...
         if (!isGroup) return fakestatus(mess.only.group)
          ...
          if (!isGroupAdmins) return fakestatus(mess.only.admin)
              ...
              break
          case 'infoig':
              teks = `Jangan Lupa Follow Ig Owner Ya\nhttps://www.instagram.com/imfikkur___`
              Fix.sendMessage(from, teks, text, { quoted : mek })
              break
          case 'sourcecode': 
          case 'sc': 
          case 'src':
              textImg(`Bot Ini Menggunakan Base Self Bot : https://github.com/Faarz/selfFarz`)
              break
          case 'jadibot':
          if (!isOwner && !mek.key.fromMe) return fakestatus(mess.only.owner)
...
console.log(color('|ERR|', 'red'), color(e, 'cyan'))
     Fix.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "YouTube FixMood XD",body:"",previewType:"PHOTO",thumbnail:image, sourceUrl:"https://youtu.be/mw9tozDy8ok"}}})
}
}
}
